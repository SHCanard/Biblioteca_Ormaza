...
<head>
    <title><?php echo $language['loan_list']; ?></title>
    ...
    <?php include('../templates/navbar_user.tpl'); ?>
        <div class="container">
            <div class="page-header">
              <h1 class="all-tittles"><?php echo $language['system_title']; ?> <small><?php echo $language['loan_list']; ?></small></h1>
            </div>
        </div>
        ...
        <div class="container-fluid">
            <h2 class="text-center all-tittles"><i class="zmdi zmdi-calendar zmdi-hc-fw"></i> <?php echo $language['loan_list']; ?></h2>
        </div>
        <div class="container-fluid">
			<div class="row">
                <div class="col-xs-12 lead">
                    <ol class="breadcrumb">
                      <li><!--<a href="report_loans.php">Générer en PDF</a>--> <?php if(isset($_SESSION['usertype']) AND $_SESSION['usertype']==1) echo'~ <a href="csv_report_loans.php">'.$language['csv_generate'].'</a>'; ?></li>
                    </ol>
                </div>
            </div>
            <?php
                   ...
                   include ("../db/conexion.php");
                    ...
                    $mostrar = 
                    "
                        SELECT id_loans, title_books, inventory_number_books, first_name, last_name, date_loans, date_expired_loans, date_return_loans, state_loans, phone, email FROM loans,users,books WHERE id=code_user_loans AND id_book_loans=id_books;
                    ";
            ...
            <thead>
                <tr>
                    ...
                    <th><?php echo $language['loan_date_return']; ?></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php if ($fila["date_expired_loans"]<date('Y-m-d') AND $fila["state_loans"]=='En cours') echo "<b>" ?><?php echo $fila["date_expired_loans"];?><?php if ($fila["date_expired_loans"]<date('Y-m-d') AND $fila["state_loans"]=='En cours') echo "</b>" ?></td>
                    <td><?php echo $fila["date_return_loans"];?></td>
                    <td><?php if ($fila["state_loans"]==0)
									echo $language['loan_state_pending'];
								else
									echo $language['loan_state_returned'];
								?></td>
                    <td><a href="edit_loans.php?id=<?php echo $fila["id_loans"];?>"><img src='../assets/img/editar.png' width=30 height=30></a></td>
                    <td><a href="delete_loans.php?id=<?php echo $fila["id_loans"];?>"><img src='../assets/img/eliminar.png' width=30 height=30></a></td>
                    <?php }?>
                </tr>
            </tbody>
        </table>
