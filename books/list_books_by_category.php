...
$mostrar = 
"
SELECT * FROM categories WHERE id_categories='$cat';
";
$res = $conexion->query($mostrar);
$catname = $res->fetch_assoc();
$catname = $catname['name_categories'];
?>
<!DOCTYPE html>
<html lang="<?php echo $language['lang']; ?>">
<head>
    <?php echo '<title>'.$language['book_list'].' "'.$catname.'" </title>' ?>
...
<body>
    ...
    <div class="content-page-container full-reset custom-scroll-containers">
        ...
        <div class="container">
			...
			<h1 class="all-tittles"><?php echo $language['system_title']; ?> <small><?php echo $language['book_list']." ".$catname; ?></small></h1>
            </div>
        </div>
        <div class="container-fluid">
            <ul class="nav nav-tabs nav-justified"  style="font-size: 17px;">
			<?php if(isset($_SESSION['usertype']) AND ($_SESSION['usertype']==1))
				echo '<li role="presentation"><a href="new_books.php">'.$language['book_add'].'</a></li>'; ?>
              <li role="presentation" class="active"><a href="list_books.php"><?php echo $language['book_list']; ?></a></li>
            </ul>
        </div>
        <div class="container-fluid">
            <h2 class="text-center all-tittles"><i class="zmdi zmdi-collection-item zmdi-hc-fw"></i> <?php echo $language['book_list']; ?> <?php echo '"'.$catname.'"' ?></h2>
        </div>
        <div class="container-fluid">
		<?php if(isset($_SESSION['usertype']) AND $_SESSION['usertype']==1)
			echo '<div class="row">
                <div class="col-xs-12 lead">
                    <ol class="breadcrumb">
                      <li>~ <a href="csv_report_books_by_category.php?cat='.$cat.'">'.$language['csv_generate'].'</a></li>
                    </ol>
                </div>
            </div>';?>
        </div>
        <div class="container-fluid">
            <?php
                    $mostrar = 
                    "
                        SELECT * FROM books,categories,mediums WHERE id_mediums=id_medium_books AND id_categories=id_category_books AND id_category_books='$cat';
                    ";

                    $res = $conexion->query($mostrar);
                ?>
			<div align="right"><a href="#openModalSearch"><?php echo $language['book_keywords_list']; ?></a><div id="openModalSearch" class="modalDialog"><div class="modal-dialog modal-lg"><div class="modal-header"><a href="#close" title="<?php echo $language['close']; ?>" class="close">X</a><h4 class="modal-title text-center all-tittles"><?php echo $language['book_keywords_list']; ?></h4></div><div class="modal-body"><center><?php include('../templates/keywords.tpl'); ?></center></div></div></div></div>
