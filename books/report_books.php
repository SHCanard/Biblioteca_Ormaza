...
<?php
	...
	$mostrar = 
	"
		SELECT * FROM books,categories WHERE id_category_books=id_categories order by name_categories asc;
	";
...
$prev_cat="";
	...
	{
		if($i>0 AND $prev_cat!=$fila['name_categories'])
		{
		$pdf->SetFont('Arial','',12);
		$pdf->SetX(65);
		$pdf->Cell(95,6,utf8_decode($prev_cat),0,0,'C');
		$pdf->SetX(250);
		$pdf->Cell(40,6,"Total : ".$i,0,1,'C');
		$pdf->Ln();
		$i=0;
		}
		$pdf->SetFont('Arial','',12);
		...
		$pdf->SetX(15);
		$pdf->Cell(65,6,utf8_decode($fila['title_books']),1,0,'C');
		$pdf->SetX(80);
		$pdf->Cell(80,6,utf8_decode($fila['name_categories']),1,0,'C');
		$pdf->SetX(160);
